import React from "react";
import Swyx from "../images/swyx.jpg";

const About = () => {
  return (
    <>
      <h2 className=" mt-16 capitalize text-4xl leading-10 font-display font-semibold text-gray-900 md:text-5xl md:leading-none text-center">
        About the author
      </h2>
      <div className="flex flex-col lg:flex-row w-11/12 items-center mx-auto my-16 lg:w-1/2">
        <img className="max-w-xs rounded-full " src={Swyx} alt="swyx" />
        <div>
          <p className="ml-5 mb-5  mt-12 lg:mt-0">
            Hello, I&apos;m @swyx!{" "}
            <a
              href="https://medium.com/hackernoon/no-zero-days-my-path-from-code-newbie-to-full-stack-developer-in-12-months-214122a8948f"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              I changed careers at age 30
            </a>{" "}
            , starting from doing{" "}
            <a
              href="https://medium.com/hackernoon/no-zero-days-my-path-from-code-newbie-to-full-stack-developer-in-12-months-214122a8948f"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              FreeCodeCamp
            </a>{" "}
            during nights and weekends in 2017, to getting hired as a{" "}
            <a
              href="https://medium.com/hackernoon/no-zero-days-my-path-from-code-newbie-to-full-stack-developer-in-12-months-214122a8948f"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              Senior Dev Advocate at AWS
            </a>{" "}
            in 2020.
          </p>
          <p className="ml-5 leading-6 mb-5">
            In my other pursuits, I{" "}
            <a
              href="https://swyx.io/writing"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              write
            </a>
            ,{" "}
            <a
              href="https://swyx.io/speaking"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              speak
            </a>{" "}
            and teach React, TypeScript, Storybook and Node.js (including as an{" "}
            <a
              href="https://egghead.io/instructors/shawn-wang"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              Egghead.io
            </a>{" "}
            Instructor).
          </p>
          <p className="ml-5 leading-6">
            I am a cofounder of the{" "}
            <a
              href="https://twitter.com/SvelteSociety"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              Svelte Society
            </a>{" "}
            global meetup community and up til recently served as a moderator
            for the 200k-strong{" "}
            <a
              href="https://www.reddit.com/r/reactjs/"
              target="_blank"
              rel="noreferrer"
              className=" text-blue-500"
            >
              /r/reactjs community.
            </a>{" "}
          </p>
        </div>
      </div>
    </>
  );
};

export default About;
